...
type PeriodMetric="videoViews"|"likes"|"shares"|"comments";
type TotalMetric=PeriodMetric|"fullVideoWatchedRate";
type ListSelection="all"|"new"|PeriodMetric|`total-${TotalMetric}`;
type Card={label:string;value:number;icon:typeof Video;metric:ComparisonKey;percentage?:boolean;list?:ListSelection};
...
export function Overview({onMeta:_onMeta}:{onMeta:(x:{refresh:()=>void;loading:boolean;total:number;lastSync:string|null})=>void}){
 const s=useAnalytics<SummaryAnalytics>("summary"),d=useAnalytics<DailyAnalytics[]>("daily",7),allVideos=useAnalytics<TikTokVideoRecord[]>("all-videos"),newVideos=useAnalytics<TikTokVideoRecord[]>("new-videos"),periodVideos=useAnalytics<TikTokVideoRecord[]>("period-videos"),[list,setList]=useState<ListSelection|null>(null);
 ...
 const totalCards:Card[]=[
  ...
  {label:"Tổng lượt xem",value:s.data.totalViews,icon:Eye,metric:"totalViews",list:"total-videoViews"},
  {label:"Tổng lượt thích",value:s.data.totalLikes,icon:Heart,metric:"totalLikes",list:"total-likes"},
  {label:"Tổng lượt chia sẻ",value:s.data.totalShares,icon:Share2,metric:"totalShares",list:"total-shares"},
  {label:"Tổng bình luận",value:s.data.totalComments,icon:MessageCircle,metric:"totalComments",list:"total-comments"},
  {label:"Tỷ lệ xem hết",value:s.data.averageFullWatchRate,icon:Percent,metric:"averageFullWatchRate",percentage:true,list:"total-fullVideoWatchedRate"}
 ];
 const newCards:Card[]=[
  {label:"Video đăng mới",value:s.data.newVideos,icon:Clapperboard,metric:"newVideos",list:"new"},
  {label:"Tổng lượt xem trong kỳ",value:s.data.newVideoViews,icon:Eye,metric:"newVideoViews",list:"videoViews"},
  {label:"Tổng lượt thích trong kỳ",value:s.data.newVideoLikes,icon:Heart,metric:"newVideoLikes",list:"likes"},
  {label:"Tổng lượt chia sẻ trong kỳ",value:s.data.newVideoShares,icon:Share2,metric:"newVideoShares",list:"shares"},
  {label:"Tổng bình luận trong kỳ",value:s.data.newVideoComments,icon:MessageCircle,metric:"newVideoComments",list:"comments"},
 ...
 const totalMetric=list?.startsWith("total-")?list.slice(6) as TotalMetric:null;
 const periodMetric=list!==null&&list!=="all"&&list!=="new"&&!totalMetric?list as PeriodMetric:null;
 const metricLabels:Record<TotalMetric,string>={videoViews:"lượt xem",likes:"lượt thích",shares:"lượt chia sẻ",comments:"bình luận",fullVideoWatchedRate:"tỷ lệ xem hết"};
 const metricValue=(video:TikTokVideoRecord,metric:TotalMetric)=>metric==="fullVideoWatchedRate"?(video[metric]<=1?video[metric]*100:video[metric]>100?video[metric]/100:video[metric]):video[metric];
 const metricVideos=(videos:TikTokVideoRecord[],metric:TotalMetric)=>[...videos].filter(video=>metricValue(video,metric)>0).sort((a,b)=>metricValue(b,metric)-metricValue(a,metric));
 const modalVideos=totalMetric?metricVideos(allVideos.data||[],totalMetric):periodMetric?metricVideos(periodVideos.data||[],periodMetric):(list==="all"?allVideos.data:newVideos.data)||[];
 const modalTitle=totalMetric?`Video theo ${metricLabels[totalMetric]} tổng kênh`:periodMetric?`Video có ${metricLabels[periodMetric]} trong kỳ`:list==="all"?"Chỉ số tổng kênh":"Chỉ số trong khoảng thời gian đã chọn";
 const sortLabel=totalMetric?metricLabels[totalMetric]:periodMetric?metricLabels[periodMetric]:undefined;
 return <div className="space-y-6"><div className="border-b border-slate-200 pb-5"><p className="mb-1 text-xs font-bold uppercase tracking-[.12em] text-brand-700">Báo cáo kênh</p><h1 className="text-2xl font-bold tracking-tight text-slate-950 md:text-[28px]">Tổng quan hiệu suất</h1><p className="mt-1.5 text-sm text-slate-500">Hiệu suất toàn kênh và video đăng mới trong kỳ đã chọn.</p></div><section><h2 className="mb-3 text-xs font-bold uppercase tracking-[.1em] text-slate-500">Chỉ số tổng kênh</h2>{row(totalCards,false)}</section><section><h2 className="mb-3 text-xs font-bold uppercase tracking-[.1em] text-slate-500">Chỉ số trong khoảng thời gian đã chọn</h2>{row(newCards)}</section><section><div className="mb-3 flex items-center justify-between"><h2 className="text-xs font-bold uppercase tracking-[.1em] text-slate-500">Biểu đồ diễn biến</h2><span className="text-xs text-slate-400">Theo ngày</span></div><div className="grid gap-5 xl:grid-cols-2"><Panel className="p-5"><h3 className="font-bold text-slate-900">Lượt xem theo ngày</h3><p className="mb-4 mt-1 text-xs text-slate-500">Chênh lệch giữa hai snapshot liên tiếp</p>{d.data?.length?<TrendChart data={d.data}/>:<Empty/>}</Panel><Panel className="p-5"><h3 className="font-bold text-slate-900">Tương tác theo ngày</h3><p className="mb-4 mt-1 text-xs text-slate-500">Likes, shares và comments phát sinh</p>{d.data?.length?<TrendChart data={d.data} metric="engagement" bar/>:<Empty/>}</Panel></div></section>{list&&<VideoListModal title={modalTitle} videos={modalVideos} loading={totalMetric?allVideos.loading:periodMetric?periodVideos.loading:list==="all"?allVideos.loading:newVideos.loading} sortLabel={sortLabel} onClose={()=>setList(null)}/>}</div>
}
...
function VideoListModal({title,videos,loading,sortLabel,onClose}:{title:string;videos:TikTokVideoRecord[];loading:boolean;sortLabel?:string;onClose:()=>void}){
 const exportCsv=()=>{
